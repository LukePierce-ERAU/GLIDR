...
glidevar(:,:,ii) = {t;y(:,1);y(:,2);velo(:,1);L_D(:,1);cL(:,1)};
...
for glide_ind = 1:size(glidevar,3)
    ...
    t = free_out(1,1,glide_ind) + pull_out(1,1,glide_ind) + glide_mat_ind(1:size(glide_mat_ind,1)/6);
    x = glide_mat_ind(size(glide_mat_ind,1)/6+1:size(glide_mat_ind,1)/6*2);
    alt = glide_mat_ind(size(glide_mat_ind,1)/6*2+1:size(glide_mat_ind,1)/6*3);
    speed = glide_mat_ind(size(glide_mat_ind,1)/6*3+1:size(glide_mat_ind,1)/6*4);
    L_D = glide_mat_ind(size(glide_mat_ind,1)/6*4+1:size(glide_mat_ind,1)/6*5);
    cL = glide_mat_ind(size(glide_mat_ind,1)/6*5+1:end);
...
while alt > 1700
    ...
    % Drag Buildup for changing Re and flight condition
    Re = rho.*DESIGN.c.*V./mu;
    [cD0,cDi] = drag(Re,cL(iter,1),DESIGN,ii);
    cD = (cD0 + cDi);

    L_Dtemp = cL(iter,1)/cD;
    gamma = atan(cD/cL(iter,1));
